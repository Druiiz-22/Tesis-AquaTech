...
public class Recuperacion extends JPanel implements properties.Colores, properties.Constantes {
    ...
    public static void cambiarClave() {
        ...
        //Validar que ningún atributo esté vacío
        if (!correo.isEmpty()) {
            if (!claveNueva.isEmpty()) {

                //Intentar cambiar la contraseña del usuario
                if (UpdateDB.updateUsuarioClave(correo, claveNueva)) {
                    //Mensaje de éxito
                ...
                msjError("La clave nueva para restaurar, es inválida." + error);
            }
        } else {
            msjError("El correo del usuario, es inválido." + error);
        }
    }
