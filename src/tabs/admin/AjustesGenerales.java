...
public class AjustesGenerales extends JScrollPane implements properties.Constantes {
    ...
    private void guardar(boolean oeste){
        if(validarCampos(oeste)){
            if(validarDatos(oeste)){
                if(oeste){
    ...
    private boolean validarCampos(boolean oeste) {
        ...
        if(oeste){
            trasvasos = txtNombre.getText().toUpperCase();
            ventas = txtRIF.getText().toUpperCase();
    ...
    private static final CampoTexto txtNumero = new CampoTexto("Numero de la cuenta", CUALQUIER);
